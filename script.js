  ...
  attack(){
    if (currentWeaponType === 1){
      ...
      if(bossFight === true && boss.length > 0){
        let distance = dist(this.x, this.y, boss[0].x, boss[0].y);
        if (distance <= this.weapon.range + 20) { // add 20, player circle center radius
          if(this.weapon.attackCoolDown === 0){
            boss.health -= this.weapon.damage;
          }
          else if(this.weapon.attackCoolDown <= 25){
            boss.health -= this.weapon.damage / 3; // light hit
          }
        }
...
class Boss extends Creatures{
  ...
  callZombie(){
    if(zombies.length === 0){
      for(var i = 0; i < 4; i++){
        if(difficulty === 0){
          zombies.push(new Zombie(player.x + 50, player.y + 25, 10, 60, 0.5, 2.5));
        }else if(difficulty === 1){
          zombies.push(new Zombie(player.x + 50, player.y + 25, 11, 60, 0.5, 3));
        }else if(difficulty === 2){
          zombies.push(new Zombie(player.x + 50, player.y + 25, 16, 60, 0.5, 4.5));
        }
      }
    }else{
